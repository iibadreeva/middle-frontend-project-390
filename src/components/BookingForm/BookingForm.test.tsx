...
import { BOOKING_REQUIRED_ERROR } from '../../lib/messages';
...
describe('BookingForm', () => {
  ...
  it('marks invalid fields with aria-invalid and describedby', async () => {
    ...
    const error = screen.getByTestId('booking-error');
    const errorId = error.getAttribute('id');
    expect(errorId).toBeTruthy();
    expect(errorId).not.toBe('booking-error');
  ...
  it('gives each form instance a unique error id', async () => {
    const user = userEvent.setup();
    render(
      <>
    ...
    const errors = screen.getAllByTestId('booking-error');
    const ids = errors.map((node) => node.getAttribute('id'));
    expect(ids[0]).toBeTruthy();
    expect(ids[1]).toBeTruthy();
    expect(ids[0]).not.toBe(ids[1]);
  });

  it('clears only the edited field from invalid markers', async () => {
    const user = userEvent.setup();
    render(<BookingForm onSubmit={vi.fn()} />);
  ...
  it('hides the form error after the last invalid field is edited', async () => {
    const user = userEvent.setup();
    render(
      <BookingForm
        onSubmit={vi.fn()}
