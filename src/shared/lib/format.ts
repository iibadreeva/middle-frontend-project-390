...
function formatUtcOffsetSuffix(offsetMinutes: number): string {
  if (offsetMinutes === 0) {
    return 'UTC';
  }
...
function getTimeZoneOffsetMinutes(date: Date, timeZone: string): number {
  ...
  return parseGmtOffsetMinutes(raw);
}

/** Короткий суффикс: MSK / YEKT из словаря; иначе UTC±H[:MM]. */
function formatTimeZoneSuffix(date: Date, timeZone: string): string {
  return (
    TIME_ZONE_ABBREVIATIONS[timeZone] ??
    formatUtcOffsetSuffix(getTimeZoneOffsetMinutes(date, timeZone))
  );
}
...
export function formatDateTime(iso: string, timeZone: string): string {
  ...
  const formatted = getCachedDateTimeFormat('ru-RU', {
    day: '2-digit',
    month: '2-digit',
    year: 'numeric',
    hour: '2-digit',
