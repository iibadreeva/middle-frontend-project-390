...
function renderCard(
  ...
  const flightUnderTest = props.flight ?? flight;
  return render(
    <TestProviders cities={props.cities ?? fixtureCities}>
      <MemoryRouter>
        <FlightCard
          flight={flightUnderTest}
          passengers={props.passengers ?? 1}
          bookHref={props.bookHref ?? `/booking/${flightUnderTest.id}`}
        />
      </MemoryRouter>
    </TestProviders>,
  );
...
describe('FlightCard', () => {
  ...
  it('keeps booking enabled when seats are enough', () => {
    renderCard({ passengers: 3, bookHref: `/booking/${flight.id}` });
  ...
  it('uses the bookHref path from the composition layer', () => {
    renderCard({ bookHref: '/custom-booking/fl_x' });

    expect(screen.getByTestId('book-flight')).toHaveAttribute(
      'href',
      '/custom-booking/fl_x',
    );
  });
